...
static void handle_request(Request* request) {
    ...
    switch (request->which_payload) {
        ...
        case Request_write_tag:
            ...
            // Need to write 256 B at a time. Put our 128 B at the right offset then fill the rest
            // of the buffer with 0xFF so we don't zero the rest of the page.
            memcpy(flash_write_buffer +
                       (request->payload.write.offset % FLASH_PAGE_SIZE),
                   request->payload.write.data, WRITE_SIZE);
            memset(flash_write_buffer + WRITE_SIZE -
                       (request->payload.write.offset % FLASH_PAGE_SIZE),
                   0xFF, WRITE_SIZE);
            ...
            uint32_t page_start_offset =
                ((request->payload.write.offset / FLASH_PAGE_SIZE) *
                 FLASH_PAGE_SIZE);
            flash_range_program(APP_START_FLASH_OFFSET + page_start_offset,
                                flash_write_buffer, FLASH_PAGE_SIZE);
            break;
        case Request_go_tag:
            jump_to_app();
            break;
    ...
    while (true) {
        ...
        while (tud_cdc_available()) {
            ...
            uint32_t count = tud_cdc_read(
                rx_buffer + packet_size - remaining_bytes, remaining_bytes);
            ...
            pb_istream_t stream =
                pb_istream_from_buffer(rx_buffer, packet_size);
