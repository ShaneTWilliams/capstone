...
def type_from_field(config, driver, field_config):
    if "type" in field_config:
        if field_config["type"] in {"bool", "uint8_t", "uint16_t", "uint32_t", "uint64_t"}:
            return field_config["type"]
        if field_config["type"] in config["enums"]:
            return f"{driver.lower()}_{field_config['type'].lower()}_t"
        raise Exception(f"Invalid type: {field_config['type']}")
    return type_from_size(field_config["size"])
...
def generate_diagram(reg_name, reg_config, header):
    header.write("┌─\n")
    header.write(f"│    {reg_name} @ 0x{reg_config['address']}\n")
...
def generate_driver(path, header_dir):
    ...
    generate_comment_block("FIELD ENUMS", header)

    for enum_name, enum_config in config["enums"].items():
        header.write(f"typedef enum {{\n")
        for name, value in enum_config.items():
            header.write(f"   {driver_name.upper()}_{enum_name.upper()}_{name.upper()} = 0x{value:02},\n")
        header.write(f"}} {driver_name.lower()}_{enum_name.lower()}_t;\n\n")

    for reg_name, reg_config in config["registers"].items():
        generate_comment_block(f"REGISTER: {reg_name}", header)
        generate_comment_block(f"REGISTER: {reg_name}", source)
        header.write(f"#define {reg_name}_ADDRESS 0x{reg_config['address']}\n")
        ...
        for field_name, field_config in reg_config["fields"].items():
            read_signature += f", {type_from_field(config, driver_name, field_config)} *{field_name.lower()}"
        write_signature = f"{io_status_type} {driver_name.lower()}_write_{reg_name.lower()}({dev_type} *dev"
        for field_name, field_config in reg_config["fields"].items():
            write_signature += f", {type_from_field(config, driver_name, field_config)} {field_name.lower()}"
        ...
        source.write(
            f"{io_status_type} ret = dev->operations.read(dev->address, {reg_name}_ADDRESS, &value, {reg_config['size']});\n"
        )
